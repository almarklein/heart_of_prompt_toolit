...
from prompt_toolkit.win32_types import CONSOLE_SCREEN_BUFFER_INFO, STD_OUTPUT_HANDLE, STD_INPUT_HANDLE, COORD
...
def is_conemu_ansi():
    if not sys.platform.startswith('win'):
        return False
    return os.getenv('ConEmuANSI', 'OFF') == 'ON'
...
class Win32Output:
   ...
   ##   def encoding(self):
    ...
    def make_vt100_wrapping_flush(self, vt100_flush):
        
        # Remember the previous console mode.
        original_mode = DWORD(0)
        windll.kernel32.GetConsoleMode(self.hconsole, byref(original_mode))
        # Enable processing of vt100 sequences.
        # See: https://msdn.microsoft.com/pl-pl/library/windows/desktop/ms686033(v=vs.85).aspx
        ENABLE_PROCESSED_INPUT = 0x0001
        ENABLE_VIRTUAL_TERMINAL_PROCESSING = 0x0004
        vt100_mode = DWORD(ENABLE_PROCESSED_INPUT | ENABLE_VIRTUAL_TERMINAL_PROCESSING)
        
        def vt100_wrapping_flush():
            windll.kernel32.SetConsoleMode(self.hconsole, vt100_mode)
            try:
                vt100_flush()
            finally:
                windll.kernel32.SetConsoleMode(self.hconsole, original_mode)
        
        return vt100_wrapping_flush
   ...
   ##       # Scroll API
    ...
    def set_mouse_support(self, value):
        ...
        if value:
            mode = original_mode.value | ENABLE_MOUSE_INPUT
        else:
            mode = original_mode.value & ~ ENABLE_MOUSE_INPUT
        self._winapi(windll.kernel32.SetConsoleMode, handle, mode)
    ...
    def win32_refresh_window(self):
        ...
        """
        # todo: can we auto-call this somewhere?
        # Get console handle
